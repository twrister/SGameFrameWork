...
namespace vietlabs.fr2
{
 ...
 typeof(Selection).GetProperty("assetGUIDs", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static);
			...
			if (GLog.IsLogWarningEnabled) GLog.LogWarning("Unity changed ! Selection.assetGUIDs not found !");
		    return new string[0];
#endif
			}
		}
		...
		internal static void RepaintAllEditor(string className)
		{
			...
			{
#if FR2_DEV
			if (GLog.IsLogInfoEnabled) GLog.LogInfo(item.GetType().FullName);
#endif
		...
		internal static void ExportSelection()
		{
			...
			if (packageExportT == null)
			{
				Logger.Warming("Export Package Error : UnityEditor.PackageExport not found !");
				return;
			}
			...
			if (fieldInfo == null)
			{
				Logger.Warming("Export Package error : " + prop + " not found !");
				return;
			}
			...
			if (methodInfo == null)
			{
				Logger.Warming("Export Package error : BuildAssetList method not found !");
				return;
			}

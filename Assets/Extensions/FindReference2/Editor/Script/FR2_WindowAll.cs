...
namespace vietlabs.fr2
{
	...
	public class FR2_WindowAll : FR2_WindowBase, IHasCustomMenu
	{
		...
		public static bool IsShowing { get; protected set; } = false;
		...
		protected void OnCSVClick()
		{
			...
			if (IsFocusingUnused && csvSource == null)
			{
				...
				//if (csvSource != null) if (GLog.IsLogInfoEnabled) GLog.LogInfo("d : " + csvSource.Length);
			}

			if (IsFocusingUsedInBuild && csvSource == null)
			{
				csvSource = FR2_Ref.FromDict(UsedInBuild.refs);
				//if (csvSource != null) if (GLog.IsLogInfoEnabled) GLog.LogInfo("e : " + csvSource.Length);
			}

			if (IsFocusingDuplicate && csvSource == null)
			{
				csvSource = FR2_Ref.FromList(Duplicated.list);
				//if (csvSource != null) if (GLog.IsLogInfoEnabled) GLog.LogInfo("f : " + csvSource.Length);
			}
...
#if UNITY_2018_1_OR_NEWER
                    {
	                    ...
	                    try
	                    {
		                    if(AssetDatabase.TryGetGUIDAndLocalFileIdentifier(item, out guid, out fileid))
		                    {
			                    ...
			                    //if (GLog.IsLogInfoEnabled) GLog.LogInfo("guid: " + guid + "  fileID: " + fileid);
		                    }
	                    }
	                    catch{}
					}
		...
		void InitTabs()
		{
			...
			tabs.callback = new DrawCallback()
			{
				...
				AfterDraw = () =>
				{
					...
					if (GUI2.ToolbarToggle(ref sp1.splits[2].visible, FR2_Icon.Favorite.image, Vector2.zero, "Show / Hide Bookmarks"))
					{
						sp1.CalculateWeight();
						Repaint();
					}

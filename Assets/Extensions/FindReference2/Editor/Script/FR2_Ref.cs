...
//		}
		...
		public void Draw(Rect r, IWindow window, bool showDetails)
		{
			...
			//if (GLog.IsLogInfoEnabled) GLog.LogInfo("draw scene "+ selected);
		...
		public static Dictionary<string, FR2_Ref> FindSceneInScene(GameObject[] targets)
		{
			...
			for (var i = 0; i < objs.Length; i++)
			{
				...
				foreach (FR2_SceneCache.HashValue item1 in item.Value)
				{
					...
					// if (GLog.IsLogInfoEnabled) GLog.LogInfo(item1.objectReferenceValue);
					GameObject ob = null;
		...
		private static void FindRefInScene()
		{
			...
			FR2_SceneCache.onReady -= FindRefInScene;
			//    if (GLog.IsLogInfoEnabled) GLog.LogInfo("Time find ref in scene " + watch.ElapsedMilliseconds);
		}
		...
		private static void ApplyFilter(Dictionary<string, FR2_Ref> refs, FR2_Asset asset)
		{
			...
			if (target == null)
			{
				//if (GLog.IsLogWarningEnabled) GLog.LogWarning("target is null");
				return;
			}
			...
			if (targetIsGameobject)
			{
				foreach (GameObject item in FR2_Unity.getAllObjsInCurScene())
				{
					if (FR2_Unity.CheckIsPrefab(item))
					{
						...
						// if (GLog.IsLogInfoEnabled) GLog.LogInfo(item.name + " itemGUID: " + itemGUID);
						// if (GLog.IsLogInfoEnabled) GLog.LogInfo(target.name + " asset.guid: " + asset.guid);
	...
	public class FR2_Ref
	{
		...
		internal List<FR2_Ref> Append(Dictionary<string, FR2_Ref> dict, params string[] guidList)
		{
			...
			{
				Logger.Warming("Cache not yet ready! Please wait!");
				return result;
			}
	...
	public class FR2_RefDrawer : IRefDraw
	{
		...
		{
			//if (GLog.IsLogInfoEnabled) GLog.LogInfo("Reset :: " + assetGUIDs.Length + "\n" + string.Join("\n", assetGUIDs));
		...
		public void RefreshSort()
		{
			...
			} else 
			{
				list.Sort((r1, r2) =>
				{
					...
					if (isMixed)
					{
						...
						if (GLog.IsLogWarningEnabled) GLog.LogWarning("Mixed compared!\n" + sb.ToString());
						#endif
			...
			#if FR2_DEBUG
			if (invalidCount > 0) if (GLog.IsLogWarningEnabled) GLog.LogWarning("Removed [" + invalidCount + "] invalid assets / objects");
			#endif

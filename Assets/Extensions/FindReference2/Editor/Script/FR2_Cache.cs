	...
	public class FR2_CacheHelper : AssetPostprocessor
	{
		...
		private static void InitListScene()
		{
			...
			foreach (EditorBuildSettingsScene scene in EditorBuildSettings.scenes)
			{
				...
				// if (GLog.IsLogInfoEnabled) GLog.LogInfo(scene.path + " " + sce);
		...
		{
			//if (BuildHelper.IsBuilding) return;	
			FR2_Cache.DelayCheck4Changes();
			//if (GLog.IsLogInfoEnabled) GLog.LogInfo("OnPostProcessAllAssets : " + ":" + importedAssets.Length + ":" + deletedAssets.Length + ":" + movedAssets.Length + ":" + movedFromAssetPaths.Length);

			if (!FR2_Cache.isReady)
			{
#if FR2_DEBUG
			if (GLog.IsLogInfoEnabled) GLog.LogInfo("Not ready, will refresh anyway !");
#endif
				return;
			}
...
#if FR2_DEBUG
				if (GLog.IsLogInfoEnabled) GLog.LogInfo("Changed : " + importedAssets[i]);
#endif
...
#if FR2_DEBUG
			if (GLog.IsLogInfoEnabled) GLog.LogInfo("New : " + importedAssets[i]);
#endif
			}
...
#if FR2_DEBUG
			if (GLog.IsLogInfoEnabled) GLog.LogInfo("Deleted : " + deletedAssets[i]);
#endif
			}
...
#if FR2_DEBUG
		if (GLog.IsLogInfoEnabled) GLog.LogInfo("Changes :: " + importedAssets.Length + ":" + FR2_Cache.Api.workCount);
#endif
...
//		}
		...
		{
			// if (GLog.IsLogInfoEnabled) GLog.LogInfo ((AssetType.FILTERS.Length & s.excludeTypes) + "  " + Mathf.Pow(2, AssetType.FILTERS.Length) ); 
	...
	public class FR2_Cache : ScriptableObject
	{
		...
		internal bool disabled
		{
			...
			set
			{
				...
				if (_disabled)
				{
					//if (GLog.IsLogWarningEnabled) GLog.LogWarning("FR2 is disabled - Stopping all works !");	
					ready = false;
		...
		{
			// if (GLog.IsLogInfoEnabled) GLog.LogInfo((_cache == null) + " " + _cache._curCacheVersion );
			if (_cache == null)
			{
				return false;
			}
		...
		{
			//if (GLog.IsLogWarningEnabled) GLog.LogWarning("Found Cache!");
		...
		{
#if FR2_DEBUG
		if (GLog.IsLogInfoEnabled) GLog.LogInfo("OnEnabled : " + _cache);
#endif
			if (_cache == null)
			{
				_cache = this;
		...
		internal void ReadFromCache()
		{
			...
			for (var i = 0; i < AssetList.Count; i++)
			{
				...
				{
#if FR2_DEBUG
					if (GLog.IsLogWarningEnabled) GLog.LogWarning("Something wrong, cache found twice <" + item.guid + ">");
#endif
					continue;
				}
		...
		internal void ReadFromProject(bool force)
		{
			...
			foreach (var p in paths)
			{
				...
				{
#if FR2_DEBUG
					if (GLog.IsLogWarningEnabled) GLog.LogWarning("Ignore asset: " + p);
#endif
					continue;
				}
...
#if FR2_DEBUG
		if (GLog.IsLogInfoEnabled) GLog.LogInfo("After checking :: WorkCount :: " + workCount + ":" + AssetMap.Count + ":" + AssetList.Count);
#endif
			Check4Work();
		}
...
//#if FR2_DEBUG
//		    if (GLog.IsLogInfoEnabled) GLog.LogInfo("RefreshAsset: " + asset.guid + ":" + workCount);
//#endif
//			
//			workCount++;
//
...
//			queueLoadContent.Add(asset);
		...
		internal void AddAsset(string guid)
		{
			...
			{
				//if (GLog.IsLogWarningEnabled) GLog.LogWarning("guid already exist <" + guid + ">");
				return;
			}
			...
			//if (GLog.IsLogWarningEnabled) GLog.LogWarning("Add - AssetList: " + AssetList.Count);
		...
		{
#if FR2_DEBUG
			if (GLog.IsLogInfoEnabled) GLog.LogInfo("Check 4 Usage");
#endif
...
#if FR2_DEBUG
			if (GLog.IsLogInfoEnabled) GLog.LogInfo(Mathf.Round(t) + " : " + progress*workCount + "/" + workCount + ":" + isReady + " ::: " + queueLoadContent.Count);
#endif
		...
		{
			//if (GLog.IsLogInfoEnabled) GLog.LogInfo("Async: " + idx);
...
#if FR2_DEBUG
			if (GLog.IsLogInfoEnabled) GLog.LogInfo("Async UsedBy: " + asset.assetPath);
#endif
		...
		public FR2_Asset Get(string guid, bool isForce = false)
		{
        ...
        internal FR2_Asset FindAsset(string guid, string fileId)
        {
            ...
            {
#if FR2_DEBUG
			if (GLog.IsLogWarningEnabled) GLog.LogWarning("Cache not ready !");
#endif
                return null;
            }
		...
		public List<FR2_Asset> FindAssets(string[] guids, bool scanFolder)
		{
			if (AssetMap == null)
			{
				Check4Changes(false);
		...
		internal List<List<string>> ScanSimilar(Action IgnoreWhenScan, Action IgnoreFolderWhenScan)
		{
			...
			foreach (KeyValuePair<string, FR2_Asset> item in AssetMap)
			{
				...
				{
					// if (GLog.IsLogWarningEnabled) GLog.LogWarning("ignore: " +item.Value.assetPath);
				...
				foreach (string ignore in FR2_Setting.s.listIgnore)
				{
					if (item.Value.assetPath.StartsWith(ignore))
					{
						...
						// if (GLog.IsLogInfoEnabled) GLog.LogInfo("ignore " + item.Value.assetPath + " path ignore " + ignore);
						break;
					}
				}
				...
				{
#if FR2_DEBUG
                    if (GLog.IsLogWarningEnabled) GLog.LogWarning("Hash can not be null! ");
#endif
					continue;
				}

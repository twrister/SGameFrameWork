	...
	public class FR2_Export
	{
		...
		public static void ExportCSV(FR2_Ref[] csvSource)
		{
			...
			if (result.Length > 0)
			{
				...
				Logger.Log("[FR2] CSV file content (" + csvSource.Length + " assets) copied to clipboard!");
			}
			else
			{
				Logger.Warming("[FR2] Nothing to export!");
			}
		}
		...
		private static void Ignore()
		{
			...
			{
				Logger.Warming("FR2 cache not yet ready, please open Window > FR2_Window and hit scan project!");
				return;
			}
...
#if FR2_DEBUG
				if (GLog.IsLogInfoEnabled) GLog.LogInfo("Changed : " + guids[i]);
#endif
		...
		private static void ExportAsset()
		{
			...
			//if (GLog.IsLogInfoEnabled) GLog.LogInfo(i + ":" + list[i] + ":" + list[i].GetType());
		...
		public static void MergeDuplicate(string guid_file)
		{
			...
			//     if (GLog.IsLogInfoEnabled) GLog.LogInfo(item.name);
			// }
			...
			{
				Logger.Warming("Invalid guid <" + guid + "> in clipboard, can not replace !");
				return;
			}
...
//				}
			...
			{
				Logger.Warming("No new asset selected to replace, must select all duplications to replace");
				return;
			}
            ...
            foreach (var item in guids_files)
            {
                ...
                if(type1 != type2)
                {
                    if (GLog.IsLogWarningEnabled) GLog.LogWarning("Cannot replace asset: " + p2 + " becase difference type");
                    continue;
                }
                if(type1 == typeof(UnityEngine.Texture2D))
                {
                    ...
                    //if (GLog.IsLogInfoEnabled) GLog.LogInfo("import type " + mainImportType);
                }
                //if (GLog.IsLogInfoEnabled) GLog.LogInfo("type: " + mainType);
#endif
                lstFind.Add(g);
			...
			{
				//if (GLog.IsLogInfoEnabled) GLog.LogInfo("FR2 Replace GUID : " + assetList[i].guid + " ---> " + guid + " : " + assetList[i].UsedByMap.Count + " assets updated");
			...
			//     }
			// }
			// if (GLog.IsLogInfoEnabled) GLog.LogInfo("Time replace guid " + wat.ElapsedMilliseconds);

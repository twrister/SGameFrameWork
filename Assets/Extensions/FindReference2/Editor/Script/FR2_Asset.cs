...
namespace vietlabs.fr2
{
	...
	public class FR2_Asset
	{
		...
		public bool IsBeUseBy => UsedByMap?.Count > 0;  //是否被引用
		public Dictionary<string, FR2_Asset> mUsedByMap => UsedByMap;
...
#if FR2_DEBUG
			if (GLog.IsLogWarningEnabled) GLog.LogWarning("Refreshing ... " + loadInfoTS + ":" + AssetDatabase.GUIDToAssetPath(guid));
			if (!m_assetPath.StartsWith("Assets"))
			{
				if (GLog.IsLogInfoEnabled) GLog.LogInfo("LoadAssetInfo: " + m_assetPath);
			}
#endif
		...
		internal void LoadFileInfo()
		{
			...
			//if (GLog.IsLogInfoEnabled) GLog.LogInfo("--> Read: " + assetPath + " --> " + m_fileInfoReadTS + "<" + m_assetChangeTS);
			m_fileInfoReadTS = FR2_Unity.Epoch(DateTime.Now);
			
			if (IsMissing)
			{
				Logger.Warming("Should never be here! - missing files can not trigger LoadFileInfo()");
				return;
			}
			...
			//if (!string.IsNullOrEmpty(m_addressable)) if (GLog.IsLogWarningEnabled) GLog.LogWarning(guid + " --> " + m_addressable);
		...
		public void AddUsedBy(string guid, FR2_Asset asset)
		{
            ...
            if(guid == this.guid)
            {
                //if (GLog.IsLogWarningEnabled) GLog.LogWarning("self used");
                return;
            }
		...
		internal void GuessAssetType()
		{
			...
			else if (REFERENCABLE_EXTENSIONS.Contains(m_extension))
			{
				...
				if (m_extension == ".asset" || isUnity || m_extension == ".spriteatlas")
				{
                    ...
                    {
                        if (GLog.IsLogWarningEnabled) GLog.LogWarning("Guess Asset Type error :: " + e + "\n" + m_assetPath); 
#else
					catch
					{
#endif
		...
		internal void LoadContent()
		{
			...
			{
#if FR2_DEBUG
            if (GLog.IsLogWarningEnabled) GLog.LogWarning("Parsing DLL not yet supportted ");
#endif
				return;
			}
		...
		public static List<FR2_Asset> FindUsage(FR2_Asset asset)
		{
			if (asset == null)
			{
				return null;
        ...
        public static List<FR2_Asset> FindUsageIncludingIndirect(FR2_Asset asset)
        {
            if (asset == null)
            {
                return null;
            ...
            {
                var indirects = FR2_Cache.Api.FindAssets(refs[i].UseGUIDs.Keys.ToArray(), true);
                foreach (var indirect in indirects)
                {
                    if (indirect != asset && !refs.Contains(indirect))
                    {
                        refs.Add(indirect);
                    }
                }
        ...
        public static List<FR2_Asset> FindUsedBy(FR2_Asset asset)
        {
            return asset.UsedByMap.Values.ToList();
        }

        public static List<FR2_Asset> FindUsedByIncludingIndirect(FR2_Asset asset)
        {
            if (asset == null)
            {
                return null;
            ...
            List<FR2_Asset> refs = asset.UsedByMap.Values.ToList();

            for (int i = 0; i < refs.Count; i++)
            {
                var indirects = refs[i].UsedByMap.Values.ToList();
                foreach (var indirect in indirects)
                {
                    if (indirect != asset && !refs.Contains(indirect))
                    {
        ...
        internal static List<string> FindUsageGUIDs(FR2_Asset asset, bool includeScriptSymbols)
		{
			...
			if (asset == null)
			{
				Logger.Warming("Asset invalid : " + asset.m_assetName);
			...
			if (showUsageIcon && HashUsedByClassesIds != null)
			{
				foreach (int item in HashUsedByClassesIds)
				{
					...
					if (content != null)
					{
						...
						{
							if (GLog.IsLogWarningEnabled) GLog.LogWarning(e);
						}
						#endif
					}
				}
		...
		internal void LoadSerialized(UnityObject target)
		{
			...
			for (var i = 0; i < props.Length; i++)
			{
				...
				//if (GLog.IsLogInfoEnabled) GLog.LogInfo("Found Reference BinaryAsset <" + assetPath + "> : " + refGUID + ":" + refObj);
				AddUseGUID(refGUID);
			}
		}
		...
		{
#if FR2_DEBUG
		    if (GLog.IsLogInfoEnabled) GLog.LogInfo("ClearUseGUIDs: " + assetPath);
#endif
		...
		internal void LoadBinaryAsset()
		{
			...
			#if FR2_DEBUG
			if (GLog.IsLogInfoEnabled) GLog.LogInfo("LoadBinaryAsset :: " + assetData + ":" + type);
			#endif
		...
		internal void LoadYAML()
		{
...
#if FR2_DEBUG
	        if (GLog.IsLogInfoEnabled) GLog.LogInfo("LoadYAML: " + assetPath);
#endif

			//if(assetPath.Contains("Myriad Pro - Bold SDF"))
			//{
			//    if (GLog.IsLogInfoEnabled) GLog.LogInfo("no ne");
			//}
			// PERFORMANCE HOG!
			...
			//	//if (GLog.IsLogInfoEnabled) GLog.LogInfo(assetName + ":" +  guid);
			//	idx = text.IndexOf("guid: ", idx + 39);
			//	if (counter++ > 100) break;
			//}

			//if (counter > 100){
			//	if (GLog.IsLogWarningEnabled) GLog.LogWarning("Never finish on " + assetName);
			//}
		}
		...
		internal void LoadYAML2()
		{
...
#if FR2_DEBUG
					if (GLog.IsLogInfoEnabled) GLog.LogInfo("AddScene: " + path);
#endif
				}
			}
			...
			try
			{
	            ...
	            {
	            	if (GLog.IsLogInfoEnabled) GLog.LogInfo(assetPath + ":" + UseGUIDsList.Count);
	            }
#endif
			}
#if FR2_DEBUG
		...
		internal void LoadFolder()
		{
            ...
            {
                if (GLog.IsLogWarningEnabled) GLog.LogWarning("LoadFolder() error :: " + e + "\n" + assetPath);
#else
			catch
			{
#endif
			...
			//if (GLog.IsLogInfoEnabled) GLog.LogInfo("Load Folder :: " + assetName + ":" + type + ":" + UseGUIDs.Count);
		}
		...
		internal bool ReplaceReference(string fromGUID, string toGUID, TerrainData terrain = null)
		{
			...
			if (IsReferencable)
			{
				...
				catch (Exception e)
				{
					state = FR2_AssetState.MISSING;
//#if FR2_DEBUG
					Logger.Warming("Replace Reference error :: " + e + "\n" + m_assetPath);
//#endif
				}
			...
			Logger.Warming("Something wrong, should never be here - Ignored <" + m_assetPath +
        ...
        {
            Logger.Log("ReplaceReference: from " + fromGUID + "  to: " + toGUID + "  toFileId: " + toFileId);
            if (IsMissing)
            {
//				if (GLog.IsLogInfoEnabled) GLog.LogInfo("this asset is missing");
                return false;
            }
            ...
            if (IsReferencable)
            {
                ...
                {
                    state = FR2_AssetState.MISSING;
//					if (GLog.IsLogInfoEnabled) GLog.LogInfo("this asset not exits");
                    return false;
                }
                        ...
                        if(line.IndexOf(fromGUID, StringComparison.Ordinal) >= 0)
                        {
							...
							if(toFileId > 0)
							{
	                            ...
	                            if(index >= 0)
	                            {
									...
									{
                                        Logger.Warming("cannot parse file");
                                        return false;
									}
                                    if (fileType.ToString().Substring(0,3) != toFileId.ToString().Substring(0,3))
                                    {
                                        //difference file type
                                        Logger.Warming("Difference file type");
                                        return false;
                                    }

                                    Logger.Log("ReplaceReference: fromFileId " + fromFileId + "  to File Id " + toFileId);

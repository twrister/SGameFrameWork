...
namespace EmojiText.Taurus
{
    ...
    public class InlineText : Text, IPointerClickHandler
    {
        ...
        private static readonly Regex _inputTagRegex = new Regex(@"\[(\-{0,1}\d{0,})#([0-9]{2})\]", 
            RegexOptions.Singleline);
        private static readonly Regex _inputTagRegex2 = new Regex(@"\[(\-{0,1}\d{0,})#(.+?)(:{1}(.+?))?\]",
            RegexOptions.Singleline);
        //文本表情管理器
        ...
        private string GetOutputText(string inputText)
        {
            ...
            foreach (Match match in _inputTagRegex.Matches(inputText))
            {
                ...
                int emojiId = int.Parse(match.Groups[2].Value);

                //Logger.Log("inputText = {0}, tempId = {1}, emojiId = {2}", inputText, tempId, emojiId);

Shader "Unlit/RingSkillIndicatorSimpleFade"
{
    Properties
    {
        ...
        _CenterFadeEndRatio      ("Center Fade End Ratio", Range(0,1)) = 1.0
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float centerFadeEnd = _OuterRadius * _CenterFadeEndRatio;
                float centerFadeStart = centerFadeEnd - _CenterFadeDistance;
                if (_CenterFadeDistance > 0 && r >= centerFadeStart)
                {
                    float cf = saturate((centerFadeEnd - r) / _CenterFadeDistance);
                    result.a *= cf;
                }

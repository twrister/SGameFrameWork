...
namespace SthGame.EffectMgrDemo
{
    ...
    public class EffectMgr : MonoBehaviour
    {
        ...
        public Transform Play(GameObject effectPrefab, Vector3 pos, Quaternion rot)
        {
            ...
            //判断视野外不播放
            if (CheckPosOutOfView(pos) && CheckCanIgnore(effectPrefab))
            {
                return null;
            }
        ...
        public Transform Play(GameObject effectPrefab, Transform parent)
        {
            if (effectPrefab == null) return null;
            //判断视野外不播放
            if (CheckPosOutOfView(parent.position) && CheckCanIgnore(effectPrefab))
            {
                return null;
            }

            GameObject effect = GameObject.Instantiate(effectPrefab, parent);
        ...
        private bool CheckPosOutOfView(Vector3 pos)
        {
            return false;
        }
